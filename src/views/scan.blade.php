...
<body>
    ...
    <div class="container-fluid p-0">
        ...
        <ul class="nav nav-tabs nav-justified" style="border-bottom-color: #111111 !important;">
            <li class="nav-item"><a class="nav-link active" id="scan_card_tab" data-toggle="tab" href="#scan_tab">Scan
                    Card</a></li>
            <li class="nav-item"><a class="nav-link" id="upload_card_tab" data-toggle="tab" href="#upload_tab">Upload
                    Card</a></li>
        </ul>
            ...
            <div class="tab-pane fade" id="upload_tab">
                <div class="container">
                    <div class="upload-box">
                        ...
                        <input type="file" id="upload_input" name="mobicard_scan_card_photo"
                            class="form-control mt-3" accept="image/*,application/pdf"
                            style="padding: .375rem .75rem 2.2rem;">
            ...
            <div class="modal-content text-dark">
                ...
                <div class="modal-body">
                    <div class="mb-2">
                        ...
                        <input type="text" id="res_card_number_display" class="form-control" autocomplete="off"
                            inputmode="numeric" placeholder="Card number" required>
                    </div>

                    <div class="mb-2">
                        <div class="result-label expiry-label">Expiry</div>

                        <input type="text" id="res_card_expiry" class="form-control" autocomplete="off"
                            placeholder="MM/YY" pattern="(0[1-9]|1[0-2])\/([0-9]{2})"
                            title="Please enter a valid date in MM/YY format" maxlength="5" required>
                    </div>
                    ...
                    <div class="mb-2">
                        ...
                        <input type="tel" id="res_card_cvv" class="form-control" inputmode="numeric"
                            pattern="[0-9]{3,4}" maxlength="4" autocomplete="off" required
                    ...
                    <div class="row g-0 w-100">
                        <div class="col-6">
                            <button class="btn btn-secondary w-100 h-100 rounded-0 rounded-start"
                                style="border-radius: 10px 10px 10px 10px !important;" data-dismiss="modal"
                                id="close_modal_btn">
                                Close
                            </button>
                        </div>
    ...
    <script>
        ...
        const mobicard_scan_card_url = "{{ $mobicard_scan_card_url }}";
        const mobicard_transaction_access_token = "{{ $mobicard_transaction_access_token }}";
        const mobicard_token_id = "{{ $mobicard_token_id }}";
        ...
        function cleanupAllTimers() {
            ...
            autoSubmitTimeoutIds.forEach(timeoutId => clearTimeout(timeoutId));
            autoSubmitTimeoutIds = [];
        }

        function resetIdleTimer() {
            if (idle_timer) clearTimeout(idle_timer);
            idle_timer = setTimeout(showLockScreen, 45000);
        }
                            ...
                            (function(iteration) {
                                const timeoutId = setTimeout(function() {
                                    if (autoSubmitActive && retryCount < MAX_RETRIES) {
                                        debugLog(
                                            `Running capture_and_submit() - iteration ${iteration + 1}/5`
                                            );
                                        capture_and_submit();
                                    }
                                }, iteration * 2000);
        ...
        $('#proceed_btn').on('click', function(e) {
            ...
            if (ajaxResponse) {
                alert(JSON.stringify(ajaxResponse, null, 2));
            } else {
        ...
        // ================= SUBMIT SCAN (via Laravel broker) =================
        function submit_form_data(file_blob) {
            ...
            reader.onloadend = function() {
                ...
                const requestData = {
                    mobicard_transaction_access_token: mobicard_transaction_access_token,
                    mobicard_token_id: mobicard_token_id,
                    mobicard_scan_card_url: mobicard_scan_card_url,
                    image_data: base64data,
                    file_type: file_blob.type ? file_blob.type.split('/')[1] : 'jpg',
                    file_name: 'scan.' + (file_blob.type ? file_blob.type.split('/')[1] : 'jpg')
                };

                $.ajax({
                    url: '/mobicard/broker/scan',
                    type: 'POST',
                    ...
                    success: function(resp) {
                        ...
                        if (success_response_flag === 0) {

                            if (resp && (resp.status_code === '429' || resp.status_code === 429)) {
                                stopCamera();
                                cleanupAllTimers();
                                showLockScreen();
                    ...
                    error: function(xhr, status, error) {
                        ...
                        if (success_response_flag === 0) {
                            ...
                            if (xhr.status === 429 || (xhr.responseJSON && xhr.responseJSON.status_code ===
                                    '429')) {
                                stopCamera();
                                cleanupAllTimers();
                                showLockScreen();
                            ...
                            if (xhr.status === 430 || (xhr.responseJSON && xhr.responseJSON.status_code ===
                                    '430')) {
        ...
        // ================= SUBMIT UPLOAD (via Laravel broker) =================
        function submit_form_data_upload(file_blob) {
            ...
            formData.append('mobicard_scan_card_url', mobicard_scan_card_url);

            $.ajax({
                url: '/mobicard/broker/upload',
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
                ...
                success: function(resp) {
                    ...
                    if (success_response_flag === 0) {
                        ...
                        if (resp && resp.status_code && resp.status_code !== '200' && resp.status_code !==
                            'SUCCESS') {
